...
统计提交会在服务端重新校验作品 ID、评分胜负和分差，不采用前端上报的正确率或胜负结果。服务端只保存聚合数据：

- `distribution`：按题库、模式和题数/时长保存正确率直方图桶，后台展示和赛后水平对比默认按题库与模式汇总
- `pair`：按题库、模式和作品组合聚合的出现次数、正确次数、错误次数、双方被选择次数、双方作为正确答案时的答对次数
- `consent`：匿名弹窗展示、同意和拒绝计数，用于估算统计样本来源

题目难度以作品组合为单位统计，避免把单部作品从配对关系中抽离后直接下结论。后续可用于分析“某一组题是否容易误判”“同一作品在不同配对中表现是否不同”“大分差题组是否仍有高错误率”等问题。

统计后台入口为 `/#admin`。后台数据接口使用 Cloudflare Worker Secret `ADMIN_TOKEN` 鉴权，只返回聚合图表和题目组合汇总，不返回单局原始记录。
